...
- **Phases 1.5-1.6 are complete; Phase 1.7 is Matchup CRUD complete (2026-08-16).** Matchup creation/deletion validates both Characters exist in-game, rejects attacker == defender, rejects an optional `stageKey` that doesn't exist, rejects duplicate attacker+defender+name identity, and maintains bidirectional `Game.hierarchy.matchupKeys` linkage. A Material `MatchupEditor` (attacker/defender/optional-stage selects + name input, routed at `/matchups`) lets users create and delete Matchups. Scenario CRUD, Response CRUD, and response-tree navigation (the harder remaining deliverables) are not yet implemented.
...
**Status: Matchup CRUD complete (2026-08-16).** Matchup creation/deletion derives a semantic key from game + attacker + defender + name, rejects a Matchup where attacker and defender are the same Character, validates both Characters (and an optional `stageKey`) exist, rejects duplicate attacker+defender+name identity, and maintains bidirectional `Game.hierarchy.matchupKeys` linkage. A Material `MatchupEditor` lets users pick an attacker/defender/optional stage and a name, then create or delete Matchups; it's routed at `/matchups` with a nav entry. Scenario definition, response trees, and response-tree navigation remain.
