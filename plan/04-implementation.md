...
- Entity notes are generic: `EntityMetadata.notes` is a list of reusable `NoteEntry` values (`id`, `text`, `createdAt`, optional `promotedToKey`) rather than a single string, so every entity gets the same scratchpad model without entity-specific note types. The data facade exposes generic `addEntityNote`, `removeEntityNote`, and `promoteEntityNote` mutations keyed by `EntityType` + entity key. `NotesList` lives in `libs/app/ui/src/lib/notes-list/` as a presentational component with add/remove/promote outputs; `EntityNotes` lives in `libs/app/feature/src/lib/entity-notes/` as the shared facade-aware wrapper that renders a compact Notes button and expands the notes list inline, owns add/remove/error handling, and emits promotion to its parent editor. Entity list rows use native summary/details semantics: the summary is the entity's semantic identity, and the expanded details contain Notes first followed by child entity data and row actions. Game, Stage, Stage Zone, Character, Move, Sequence, Team, and Matchup editors now expose this hierarchy; promotion is opt-in and currently enabled only for Matchups, where a quick note can be kept as history, promoted into the Scenario draft with its text as the Scenario name, and linked to the created Scenario once the user selects the required opponent Move/Sequence. The next notes follow-up is wiring the same promotion pattern from Scenario notes into formal Scenario Responses; Character→Move and Stage→Zone promotion are plausible later extensions but are intentionally not specialized yet.
